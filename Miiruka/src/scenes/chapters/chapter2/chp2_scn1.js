...
export class Chp2_scn1 extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image(
      "molino-base",
      "assets/juegos/molino/molino_con_bomba_sin_aspas.png",
    );
  ...
  create() {
    ...
    // Audio de ambiente.
    this.birdsSounds = this.sound.add("birds", {
      volume: 1,
    });
    this.birdsSounds.play();
    this.chirridoSound = this.sound.add("chirrido", {
      volume: 0.12,
      loop: true,
    });
    ...
    const layer1 = this.add
      .tileSprite(960, 1230, 1920, 1080, "bg_layer1")
      .setScrollFactor(0.7);
    const layer2 = this.add
      .tileSprite(960, 1260, 1920, 1080, "bg_layer2")
      .setScrollFactor(0.8);
    const layer3 = this.add
      .tileSprite(960, 1300, 1920, 1080, "bg_layer3")
      .setScrollFactor(0.9);
    const layer4 = this.add
      .tileSprite(960, 1340, 1920, 1080, "bg_layer4")
      .setScrollFactor(1);
    ...
    this.time.delayedCall(0, async () => {
      this.placeMill();
      await this.storyRunner.run("Inicio");
    });
  }
  ...
  placeMill() {
    ...
    const molinoBase = this.add
      .image(baseX, baseY, "molino-base")
      .setOrigin(0, 0)
      .setScale(baseScale);
    ...
    const aspas = this.add
      .image(aspasX, aspasY, "molino-aspas")
      .setOrigin(0.5, 0.5);
    aspas.setDepth(130);

    const movingPiece = this.add
      .image(baseX + 703, baseY + 1736, "moving-piece")
      .setOrigin(0.5, 1);
  ...
  updateMovingPieceFaulty(delta, spinSpeed) {
    ...
    this.movingPiecePhase +=
      dt * Phaser.Math.Clamp(1 + speedAbs * 0.45, 0.9, 3.2);
    ...
    const eased = Phaser.Math.Easing.Sine.InOut(
      Phaser.Math.Clamp(upDown, 0, 1),
    );
    const amplitude = Phaser.Math.Clamp(speedAbs / 1.6, 0.12, 1);
    const lift = eased * amplitude;
    this.movingPiece.y = Phaser.Math.Linear(
      this.movingPieceBaseY,
      this.movingPieceTopY,
      lift,
    );
  }
  ...
  update(time, delta) {
    ...
    if (this.molinoAspas) {
      ...
      if (t < 0.34) {
        ...
        speed = Phaser.Math.Linear(1.55, -0.48, (t - 0.34) / 0.1);
      } else if (t < 0.68) {
        speed = Phaser.Math.Linear(-0.48, 1.35, (t - 0.44) / 0.24);
      } else if (t < 0.78) {
        speed = Phaser.Math.Linear(1.35, -0.42, (t - 0.68) / 0.1);
      } else {
